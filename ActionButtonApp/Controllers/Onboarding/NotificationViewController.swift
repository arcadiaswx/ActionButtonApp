...
class NotificationViewController: OnboardingViewController, UIActionSheetDelegate {
    ...
    lazy var fromActionSheet: AHKActionSheet = {
        ...
        for index in 5...22 {
            actionSheet.addButtonWithTitle("\(index):00", type: .Default) { _ in
                self.userDefaults.setInteger(index, forKey: ComplexConstants.Notification.From.key())
    ...
    lazy var intervalActionSheet: AHKActionSheet = {
        ...
        for index in 1...8 {
            ...
            actionSheet.addButtonWithTitle("\(index) \(hour)", type: .Default) { _ in
                self.userDefaults.setInteger(index, forKey: ComplexConstants.Notification.Interval.key())
                self.userDefaults.synchronize()
                self.updateUI()
            }
        }
    ...
    override func updateUI() {
        self.fromLabel.text = "\(self.userDefaults.integerForKey(ComplexConstants.Notification.From.key())):00"
        self.toLabel.text = "\(self.userDefaults.integerForKey(ComplexConstants.Notification.To.key())):00"
        let interval = self.userDefaults.integerForKey(ComplexConstants.Notification.Interval.key())
        let hour = interval > 1 ? NSLocalizedString("hours", comment: "") : NSLocalizedString("hour", comment: "")
        self.intervalLabel.text = "\(interval) \(hour)"
        self.notificationSwitch.on = self.userDefaults.boolForKey(ComplexConstants.Notification.On.key())
    }
    ...
    @IBAction func openToSelection(sender: UIButton) {
        ...
        for index in upper...24 {
            toActionSheet!.addButtonWithTitle("\(index):00", type: .Default) { _ in
                self.userDefaults.setInteger(index, forKey: ComplexConstants.Notification.To.key())
                self.userDefaults.synchronize()
                self.updateUI()
            }
        }
    ...
    @IBAction func enableNotifications(sender: UISwitch) {
        self.userDefaults.setBool(sender.on, forKey: ComplexConstants.Notification.On.key())
        self.userDefaults.synchronize()
    }

    @IBAction func doneAction() {
        userDefaults.setBool(true, forKey: ComplexConstants.General.OnboardingShown.key())
