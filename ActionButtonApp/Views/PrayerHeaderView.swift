...
struct PrayerHeaderButtons : OptionSetType {
    let rawValue : Int
    init(rawValue: Int) { self.rawValue = rawValue }

    static let None = PrayerHeaderButtons(rawValue: 1 << 0)
    static let Like = PrayerHeaderButtons(rawValue: 1 << 1)
    static let Comment = PrayerHeaderButtons(rawValue: 1 << 2)
    static let User = PrayerHeaderButtons(rawValue: 1 << 3)
    
    static let Default: PrayerHeaderButtons = [Like, Comment, User]
}

class PrayerHeaderView: PFTableViewCell {

    /// The bitmask which specifies the enabled interaction elements in the view
    var buttons: PrayerHeaderButtons = .None

    /*! @name Accessing Interaction Elements */

    /// The Like Prayer button
    var likeButton: UIButton?
    
    /// The Comment On Prayer button
    var commentButton: UIButton?
    
    var delegate: PrayerHeaderViewDelegate?
    ...
    init(frame: CGRect, buttons otherButtons: PrayerHeaderButtons) {
        super.init(style: UITableViewCellStyle.Default, reuseIdentifier: nil)
        self.frame = frame
        
        PrayerHeaderView.validateButtons(otherButtons)
...
// FIXME: It crashed!        self.superview!.clipsToBounds = false
        ...
        if self.buttons.contains(PrayerHeaderButtons.Comment) {
            // comments button
        ...
        if self.buttons.contains(PrayerHeaderButtons.Like) {
            // like button
        ...
        if self.buttons.contains(PrayerHeaderButtons.User) {
    ...
    /// The prayer associated with this view
    var prayer: PFObject? {
        didSet {
            // user's avatar
            let user: PFUser? = prayer!.objectForKey(kPrayerUserKey) as? PFUser
            ...
            if self.buttons.contains(PrayerHeaderButtons.Comment) {
                ...
                self.commentButton!.addTarget(self, action: Selector("didTapCommentOnPrayerButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
            }
            
            if self.buttons.contains(PrayerHeaderButtons.Like) {
                constrainWidth = self.likeButton!.frame.origin.x
                self.likeButton!.addTarget(self, action: Selector("didTapLikePrayerButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
            }
            ...
            let timeInterval: NSTimeInterval = self.prayer!.createdAt!.timeIntervalSinceNow
    ...
    func shouldEnableLikeButton(enable: Bool) {
        if enable {
            self.likeButton!.removeTarget(self, action: Selector("didTapLikePrayerButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        } else {
            self.likeButton!.addTarget(self, action: Selector("didTapLikePrayerButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        }
    }
    ...
    static func validateButtons(buttons: PrayerHeaderButtons) {
        if buttons == PrayerHeaderButtons.None {
// FIXME            [NSException raise:NSInvalidArgumentException format:@"Buttons must be set before initializing PrayerHeaderView."];
            fatalError("Buttons must be set before initializing PrayerHeaderView.")
        }
    }
    
    func didTapUserButtonAction(sender: UIButton) {
        if delegate != nil && delegate!.respondsToSelector(Selector("prayerHeaderView:didTapUserButton:user:")) {
            delegate!.prayerHeaderView!(self, didTapUserButton: sender, user: self.prayer![kPrayerUserKey] as! PFUser)
        }
    }
    
    func didTapLikePrayerButtonAction(button: UIButton) {
        if delegate != nil && delegate!.respondsToSelector(Selector("prayerHeaderView:didTapLikePrayerButton:prayer:")) {
            delegate!.prayerHeaderView!(self, didTapLikePrayerButton: button, prayer: self.prayer!)
        }
    }
    
    func didTapCommentOnPrayerButtonAction(sender: UIButton) {
        if delegate != nil && delegate!.respondsToSelector(Selector("prayerHeaderView:didTapCommentOnPrayerButton:prayer:")) {
            delegate!.prayerHeaderView!(self, didTapCommentOnPrayerButton: sender, prayer: self.prayer!)
        }
    }
...
/*!
 The protocol defines methods a delegate of a PrayerHeaderView should implement.
 All methods of the protocol are optional.
 */
@objc protocol PrayerHeaderViewDelegate: NSObjectProtocol {
    /*!
     Sent to the delegate when the user button is tapped
     @param user the PFUser associated with this button
     */
    optional func prayerHeaderView(prayerHeaderView: PrayerHeaderView, didTapUserButton button: UIButton, user: PFUser)

    /*!
     Sent to the delegate when the like prayer button is tapped
     @param prayer the PFObject for the prayer that is being liked or disliked
     */
    optional func prayerHeaderView(prayerHeaderView: PrayerHeaderView, didTapLikePrayerButton button: UIButton, prayer: PFObject)

    /*!
     Sent to the delegate when the comment on prayer button is tapped
     @param prayer the PFObject for the prayer that will be commented on
     */
    optional func prayerHeaderView(prayerHeaderView: PrayerHeaderView, didTapCommentOnPrayerButton buton: UIButton, prayer: PFObject)
}