...
class FindFriendsCell: PFTableViewCell {
    ...
    var user: PFUser? {
        didSet {
            ...
            // Set name 
            let nameString: String = self.user!.objectForKey(kUserDisplayNameKey) as! String
            let nameSize: CGSize = nameString.boundingRectWithSize(CGSizeMake(144.0, CGFloat.max),
                                                         ...
                                                         attributes: [NSFontAttributeName: UIFont.boldSystemFontOfSize(16.0)],
                                                            context: nil).size
            nameButton.setTitle(self.user!.objectForKey(kUserDisplayNameKey) as? String, forState: UIControlState.Normal)
            nameButton.setTitle(self.user!.objectForKey(kUserDisplayNameKey) as? String, forState: UIControlState.Highlighted)
...
@objc protocol FindFriendsCellDelegate: NSObjectProtocol {
    /*!
     Sent to the delegate when a user button is tapped
     @param aUser the PFUser of the user that was tapped
     */
    func cell(cellView: FindFriendsCell, didTapUserButton aUser: PFUser)
    func cell(cellView: FindFriendsCell, didTapFollowButton aUser: PFUser)
}
